...
export const drawYAxisLabels = (arr, height, minValue, color = '#000000', yAxisLabelFontSize = 11) => {
	return (
		<View style={{
			width: 33,
			height: height,
			...
			{arr.length === 0 ? (
				...
				return (
					<View
						...
						<Text style={{ fontSize: yAxisLabelFontSize, color: color, fontWeight: '200' }}>{v[0]}</Text>
					</View>
				)
			})}
...
export const drawXAxisLabels = (sortedData, gap, color = '#000000', showEvenNumberXaxisLabel, xAxisLabelFontSize = 9, lableHeight = 10) => {
	return (
		<View style={{
			width: '100%',
			paddingVertical: 20,
				...
				if (data['x'] && i % 2 === 1 || !showEvenNumberXaxisLabel) {
					...
					return (
						<View key={'label' + i} style={{
							...
							<Text style={{ fontSize: xAxisLabelFontSize, color: color, fontWeight: '200' }}>
								{
									// data[3]
									data['x']
								}
